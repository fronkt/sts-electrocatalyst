...
Implementation commit 10dca2c62ef8546f73224848279edc2c46d1cac7 was verified on GitHub before the sixteen-file cluster transfer. All transferred bytes, the QE executable identities and six pseudopotential MD5s matched. The deployed Python 3.9 guard verified every source checkpoint and all six fresh input destinations; both shell syntax checks passed on Anvil.

Array **20513132** was submitted held, with six tasks, NP128/nk8, concurrency one, shared partition, 237G memory, four-hour limits and no requeue. All ten exclusions matched. The staged file hashes and derived manifest lines were checked again before release at **2026-09-09T01:12:28.533992+00:00** (September 8, 21:12:28 EDT).

The startup snapshot at 2026-09-09T01:13:24.037196+00:00 shows **six PENDING/Priority tasks** and no new SCF outputs. Execution startup and scientific outcomes therefore remain pending. The scheduler reports the unexpanded array as one accounting row before dispatch; the expanded six-row queue is the task-count evidence. Accounting commands in these new release/status snapshots explicitly use TZ=UTC. Full evidence is in implementation_commit.json, staging_receipt.json, held_submission.json, release.json, initial_status.json and startup_status.json under the numerical result directory.

After these results pass source/startup/SCF/projection checks, assess paired energy/force and magnetic-state changes before the next cutoff/smearing controls. The further joint-setting, memory-calibrated k-mesh, constrained relaxation and seed-1/held-out stages remain subsequent work.

## Terminal readout — September 9, 2026

All six attempts ended by **11:26:01 UTC (07:26:01 EDT)**. The live check at 22:28:19 UTC found all six terminal in Slurm accounting; the completed array had already left squeue. This establishes the state of array 20513132, not the state of other compute. The [scheduler snapshot](../../results/hea_numerical_2026-09-08/status_2026-09-09_2228/status.json) and [recomputed readout](../../results/hea_numerical_2026-09-08/status_2026-09-09_2228/readout.json) retain the evidence.

| Task | Endpoint / setting | Outcome | Final reported SCF accuracy (Ry) | Allocated core-hours |
|---|---|---|---:|---:|
| 1 | Atomic builder / tight | SCF converged; QC rejected IEEE_INVALID_FLAG | 6.6e-9 | 41.280 |
| 2 | Atomic pull / tight | Accepted SCF and projection | 7.2e-9 | 42.240 |
| 3 | Ortho builder / tight | Accepted SCF and projection | 8.5e-9 | 68.053 |
| 4 | Ortho pull / tight | Accepted SCF and projection | 9.7e-9 | 70.436 |
| 5 | Ortho pull / baseline recovery | max_seconds reached; nonconverged | 8.6243e-4 | 470.080 |
| 6 | Ortho builder / metal-alternating recovery | max_seconds reached; nonconverged | 1.3277e-4 | 470.400 |

There are **three accepted results and three rejected attempts**, with no pending tasks. Total realized allocation is **1162.488889 core-hours**, comprising 180.728889 for the accepted results, 41.280 for the exception-flagged builder and 940.480 for the two recovery attempts. These are elapsed seconds times 128 allocated CPUs, not measured utilization or a claim about billing conversion. The original two failed source states remain unresolved; neither recovery closes its missing electronic-state comparison. Scheduler logs contain an in-job seff snapshot that can still say RUNNING; final sacct records determine the terminal states.

For the accepted ortho fragment pair, **E(pull) − E(builder) = -2.129781595 eV**, compared with -2.129758057 eV in the source pair: a change of **-0.000023538 eV (-0.02354 meV)**. Builder and pull endpoint energies change by -0.02939 and -0.05293 meV, respectively. Maximum free-coordinate force-vector changes are 0.006572 and 0.008785 eV/Å. Residual free-coordinate maxima remain **1.451356 and 1.412378 eV/Å**, respectively, so these geometries remain far from stationary under the retained constraints.

Total and absolute magnetizations of both accepted ortho states are unchanged at printed precision. The largest per-atom Löwdin moment changes are 0.0024 and 0.0029 μB; HO₂ changes +1.0479→+1.0486 μB, and O₂ changes +2.0240→+2.0236 μB. The retained final Hubbard matrices provide additional finite-precision state diagnostics. This supports small sensitivity to the tighter SCF target for this sampled ortho pair. It does not establish the magnetic ground state, overall cutoff/k-mesh/smearing convergence, a calibrated error bar, or a relaxed endpoint gap. Full differences for all three accepted refinements, including the unpaired atomic pull, are in [accepted endpoint differences](../../results/hea_numerical_2026-09-08/status_2026-09-09_2228/accepted_endpoint_differences.json).

The atomic builder read the intended saved density and wavefunctions, converged in 19 SCF iterations, and printed finite forces and JOB DONE. One IEEE_INVALID_FLAG occurs in a floating-point exception summary after the timing footer (raw output line 4072). The output contains no offending rank, operation or backtrace. Footer placement does not establish when the exception occurred, and finite final values do not clear the flag. Strict QC rejected the SCF and projection did not start. The atomic tight pair therefore has **no accepted gap**; its printed energy is excluded from paired comparisons. The original source and the other five new outputs contain no invalid-operation flag.

A useful next diagnostic is a separately bounded reproduction of the atomic builder from the original verified checkpoint, preserving input, binary and MPI layout while capturing rank-attributed stderr. If the exception repeats, locate the operation with an exception-trapping diagnostic build before deciding any repair. No reproduction or further DFT has been submitted in this status check, and acceptance rules are unchanged. Repeating the two expensive ortho recoveries unchanged is not supported by the current results.

Paired cutoff and smearing controls can proceed on the accepted ortho branch while the atomic exception is diagnosed. The subsequent joint setting, memory-calibrated denser mesh and constrained fixed-cell relaxation remain necessary before extending a converged protocol to the actual seed-1 winner and held-out chains. This stage does not change the candidate ranking.

All 37 transferred files (31,680,457 bytes) match their remote SHA256 hashes. The six runtime decks match their frozen inputs, every printed pseudopotential identity matches its source (six distinct potential files, including repeated spin-labeled species), and the accepted raw outputs reproduce stored QC, startup and checkpoint-clone records. The [verification record](../../results/hea_numerical_2026-09-08/status_2026-09-09_2228/verification.json) contains cost and identity checks. The readout's REJECTED status and exit code 2 correctly represent the mixed scientific outcome; they do not mean that analysis stopped before completion. Independent calculations reproduce the accepted paired energy, force and moment changes. Original source and new scratch remain retained.
