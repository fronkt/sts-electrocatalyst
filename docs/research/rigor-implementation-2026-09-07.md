...
Implementation and evidence are committed and pushed as d667fde; the preceding selection boundary is be7d72b. Frozen snapshot loading and deck checks are portable. Snapshot regeneration embeds source paths, so changing the checkout root can change snapshot_id even when coordinates agree.
