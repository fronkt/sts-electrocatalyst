...
| P-FLOOR-U ("excess exceeds floor by >3×") | **WITHDRAWN.** The excess vanishes *identically* at the pls 3→2 crossing where ΔG₂ = ΔG₃; Cr's excess runs 0.961 → 0.007 → 0.375 V across U = 0 / 3.70 / 5.00, so the ratio is a grid artifact. Replaced by **span(c_M)/2 at fixed endpoints**, which is smooth, monotone and physical. |
