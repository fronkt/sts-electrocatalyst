...
Independent review added a live-pseudopotential safeguard before launch: all six actual UPFs must match the accepted source's SHA-256 and size before and after cloning and during the final audit. Seven ordered printed species/path/MD5 records must also match, including both oxygen-species entries. Checking only the cloned UPFs would be insufficient because setup reads the live pseudo directory. No production acceptance rule was relaxed. The regression fixtures use explicit LF bytes, including negative mutations, to exercise this Linux-output contract on Windows.

The [final diagnostic verification](../../results/hea_ieee_init_2026-09-11/verification_v3.json) passes all 50 tests and all three Bash syntax checks. Combined with the winner suite, 133 focused tests pass. The first updated fixture run exposed CRLF serialization in simulated printed-MD5 blocks; the corrected LF fixture passes without changing the production parser. Source review and tests are prerequisites for the separate live preflight, not substitutes for it.

**Launch update, 20:09 UTC:** implementation `8e7e7b7` was pushed before [verified staging](../../results/hea_ieee_init_2026-09-11/staging_receipt.json) of 31 deployment files and checks of four retained remote evidence files. Live preflight and remote shell syntax passed. [Held job 20586171](../../results/hea_ieee_init_2026-09-11/held_submission.json) matched the exact script, resource ceiling and exclusions; it was [released at 20:08:36 UTC](../../results/hea_ieee_init_2026-09-11/release.json). The [20:09:46 UTC scheduler check](../../results/hea_ieee_init_2026-09-11/startup_status.json) still showed `PENDING/Priority`, zero elapsed compute. Its actual runtime, rank-level result and diagnostic interpretation remain pending. No other new DFT calculation was submitted in this phase.
