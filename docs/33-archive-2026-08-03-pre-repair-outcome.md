> **ARCHIVED 2026-08-03 — superseded by `33-r3-mlip-evaluation.md`.**
>
> Provenance: this is docs/33 exactly as written on 2026-08-01, while the three repair
> jobs of §5 were still running. Its §3 headline — that the like-for-like model-relaxed
> test "breaks" (ρ(η) = −0.100) — was measured against a reference containing all three
> defects §4 identifies. Against the repaired reference the same MACE-MPA-0 run gives
> ρ(η) = **+0.900**, so §3's framing is now known to be a reference artefact rather than
> a model failure. §1 (the Stage 0 proof) and §4 (the defect diagnosis) stand as written.
