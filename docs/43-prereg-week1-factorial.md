...
---
## Dated addendum — 2026-09-07: approved CENSUS-1b O2-fragment completion (local MLIP arm of docs/91 §1)

The user approved the CENSUS-1b run in session on 2026-09-07, after docs/93 (commit e325dbf) reported CENSUS-1 and ENDMEMBER-2x2. This is the dated line that docs/91:90 requires before a CENSUS-1b module runs. It licenses no Anvil submission, adopts no threshold, scores no prediction, moves no registered number and fills no docs/91 slot; the arm stays reportable only as CALIBRATION under docs/43:3447-3455.

What runs: `src/scripts/site_census_o2_fragment.py` at commit e325dbf on the twelve CENSUS-1 result files whose sha256_lf docs/93 lists; MACE-MPA-0 (`macempa0mediummodel`, sha256 75428afe3a1d7d8062e19bcaabd5c433623cabf308242ec9fb493e38604fb638, checked against every input's manifest before any relaxation); float64, fmax 0.05 eV/A, 300 steps, 2 threads, the retained cell and FixAtoms of each state; the O2 molecule relaxed in each composition's slab cell with the 12 A box value recorded beside it. Site set: the 13 CENSUS-1 sites whose OOH endpoint is *O2 + H_b with the fragment bound or weak under readout (b) (`--dry-run` of 2026-09-07: 13 fragment + 12 slab-cell O2 + 1 box O2 = 26 relaxations); output only under `results/site_census_2026-09-06/o2_fragment/`. The (ZPE − TS) of the bare fragment stays the Proposed 0.05 eV and `[CENSUS-7 2026-09-__: ____]` stays unfilled; the audit records the diagnostic at 0.00, 0.05 and 0.10 eV. The diagnostic enters no ranking rule (docs/91:36, :63).

CENSUS-1b 2026-09-07: run approved on the 13 qualifying CENSUS-1 sites with src/scripts/site_census_o2_fragment.py at e325dbf; nothing licensed beyond this local run.
...
## 2026-09-21 — The Sep 20 claim-sentence re-test (run one day late), Amendment 10 adopted, and the S5 ordering correction of record

**1. Claim-sentence re-test (:1932; docs/45 §D).** The entrant ran the check on 2026-09-21 against the
2026-09-19 worksheet and evidence audit and elected the claim sentence of record:
**A tested output detector finds the detected symmetry locks, a lower bound, concentrated in the four-layer subset of a complete 810-output adsorption corpus, while full-population counts reject the registered high-exposure prediction.** It differs from the 2026-09-18 sentence by the verb only, so that what is found is the
detected (LOCKED) count, a lower bound. Verdicts of record: scorable from S1 + S2 + S6 alone; no stage
cut (P-LIT, P-XU-SPAN and the Xu molecules stay appendix rows with their own dates); C9 clean — no
sentence of A9 or of either lit-sweep synthesis is reproduced and "high-exposure prediction" occurs in
none of them; C1, C7, C8, C10, the exposure-versus-consequence rule (:1942) and graspability pass; C2,
C3, C4, C5 not engaged; counts 50 LOCKED (all four-layer), 70/810 headers as a separate count, 626 =
50/496/80 as the final-force denominator; the A9.4 displacement is the 2026-09-18 allocation. The
audit's three limits (job-class association, not thickness; no universal zero false-positive rate; four
of ten named pairs fail the energy-usability gate) travel with the sentence. The full line is in docs/45
§D. This closes the "claim sentence (Sep 20)" item of the A9 adoption status (:1950); the displacement
item closed on 2026-09-18.

**2. Amendment 10 adopted, by the A8 instrument.** The 2026-09-05 election (:4366-4373) required every
A10 threshold to be re-authored on docs/88 in the entrant's own words before A10 is appended and
deposited. On 2026-09-21 the entrant reviewed the fifteen decisions with their values in use, sources
and outcomes and adopted the operating decisions tabulated in
docs/research/research-decisions-2026-09-16.md ("A10 / S5: all fifteen decisions") as his own — his
words, from the session record of 2026-09-21: "Adopt as proposed", no exception named. By the same
authority that line supersedes the own-words requirement of :4370; A7.7's report-time rule is
untouched. The seventeen slot positions of docs/88 carry that date. Where the operating decision differs
from docs/74's proposal, the operating decision governs: A10-X, no Cr, so Ladder B (docs/74 proposed
adding Cr); A10-K, the Oct 10 submit-by date is planning history, not an automatic cut (docs/74 proposed
CUT). The docs/74 v2 text follows as AMENDMENT 10 with each THRESHOLD marker annotated in place; its
status preamble ("DRAFT v2. NOT ADOPTED, NOT DEPOSITED"; deposit by Sep 18) is superseded by the
amendment header; the Sep 18 deadline (docs/45 §D row A10) was missed.

**3. Correction of record — the S5 decks ran before A10 was adopted or deposited.** :4371 registered
that "the deposit precedes the first BEEF job". The fourteen S5 decks ran on Anvil on 2026-09-17
(results/s5_beef_2026-09-17/readout.json; the 2026-09-16 continuation, commits 3a5f707, 197e429,
33e41d0), before this adoption and before any A10 deposit, and their result was known before this
line was written: Ladder B, three scoreable metals, σ_BEEF(η) = Ru 0.1673 V, Ir 0.1994 V, Ti 0.0908 V,
all below 0.25 V, P-BEEF CONFIRMED 3/3. Facts of record: the bands (< 0.25 V confirms, ≥ 0.30 V
falsifies), the ladder, the estimator and the scope label stood in written text before the run
(docs/74 v2 of 2026-09-03; docs/88 of 2026-09-05) and none was changed after it. The entrant elected on
2026-09-21: P-BEEF remains a scored body row and carries this correction wherever it is reported —
thresholds proposed in writing before the run, adopted after it, deposited after it. This is a breach
of the registered ordering, recorded as such; it is not a clarification.
...
**Adoption record.** This amendment is research infrastructure under A7.7 (docs/74 v2, 2026-09-03;
signature sheet docs/88, 2026-09-05; operating decisions in docs/research/research-decisions-2026-09-16.md).
Its own text required every THRESHOLD to be re-authored by the entrant before deposit. What happened
instead, recorded as the authorship event by the instrument A8 used (:1467-1480): on 2026-09-21 the
entrant adopted the fifteen operating decisions as his own — his words, from the session record of
2026-09-21: "Adopt as proposed", no exception named. Under that decision every THRESHOLD marker below
is annotated in place, in brackets, with the adoption date; where the operating decision differs from
this draft's proposal (A10-X, A10-K) the bracket says so and the operating decision governs.
Adoption-time annotations change no drafted content. A7.7's model is unchanged: the draft proposes, the
entrant decided, the report paraphrases and never copies. The draft's status preamble and its Sep 18
deposit deadline are superseded by this header; the decks ran before adoption and deposit, recorded
above as a correction of record.

**Governs:** S5 — the BEEF-vdW σ row (docs/45 §E :81; stage spec round-2 :286-291) and the disposition
of P-BEEF, one of the six body-figure ledger rows (:2183).
...
> **THRESHOLD A10-E [adopted as proposed, 2026-09-21].** *Proposed:* the operative P-BEEF criterion is round-2's — its Amendment 10
...
> **THRESHOLD A10-M [adopted as proposed, 2026-09-21].** *Proposed:* any outcome in neither band maps to **SCORED — MIDDLE BAND /
...
> **THRESHOLD A10-D [adopted as proposed, 2026-09-21: Ladder B, selected by the A10-X operating decision; the ladder was in this written text before the S5 decks ran on 2026-09-17 and was not changed after].** *Proposed:* which ladder applies is fixed by the A10-X election below,
...
> **THRESHOLD A10-G [adopted as proposed, 2026-09-21; the production-binary emission the sheet left as an execution check was met on Anvil 2026-09-17: fourteen decks VALID_BEEF, 2000 members each].** *Proposed:* record gate (a) as **PASSED — SELECT-WINNER = deck (ii)
...
> **THRESHOLD A10-σ [adopted as proposed, 2026-09-21; members regenerated per A10-ENS with ASE, seed 0, sample SD with ddof = 1].** *Proposed, written longhand:*
>
...
> **THRESHOLD A10-C [adopted as proposed, 2026-09-21; the comparison values of record are Ru 0.4968, Ti 0.2459, Ir 0.0266 V and the Ir grid span 0.1979 V].** *Proposed:*
>
...
> **THRESHOLD A10-S [adopted as proposed, 2026-09-21].** *Proposed:* the S5 row is labelled **"XC only, non-magnetic three"** rather
> than round-2's "XC only", and σ_BEEF may not be generalised beyond the scoreable set without the
> extension arm having run.

> **THRESHOLD A10-X [adopted 2026-09-21 AS THE OPERATING DECISION, NOT as this draft proposed: no Cr; base {Ru, Ir, Ti}, fourteen decks, so Ladder B; and after the BEEF jobs ran, see the correction of record in the dated addendum of 2026-09-21] (extension arm — entrant's election).** *Proposed:*
...
> **THRESHOLD A10-K [adopted 2026-09-21 as the operating decision, not as proposed: the Oct 10 submit-by date is planning history, not an automatic cut; an unfinished result at a report snapshot stays unscored].** *Proposed:*
...
## 2026-09-22 — Licensed compute addition (A11.R3): two fixed-geometry SCFs on the Cu8 clean-slab stall, from the retained density; first generalization row prepared, not licensed

**Evidence this line rests on** (docs/research/lowtail-clean-slab-scf-stall-2026-09-19.md, "Diagnostic readout — 2026-09-22", and docs/research/lowtail-generalization-sizing-2026-09-22.md). Array 20813525 task 1, the Cu8Cr23Mn35Co34 seed20 site2 clean-slab relaxation, stopped at the SCF iteration ceiling in its fifth cycle with the energy stagnating at −7551.85937 Ry and an estimated accuracy near 4e-7 Ry. Array 20840139 task 2, a fresh-start SCF at the identical geometry (coordinates equal to the relaxation's printed positions after its fourth cycle to 1e-10 Å; deck differing in five lines) and the production mixing, converged at 8.08e-8 Ry to −7551.86633525 Ry, 94.7 meV lower, with the same total magnetization and Hubbard occupation traces within 0.03 electrons. Two readings remain open: a second self-consistent solution at one geometry, or a stagnating density that is not self-consistent under a misleading accuracy estimate. Task 3 of the same array, the unreconstructed-O relaxation on the same site, was killed at the ceiling after 606 iterations.

**1. Licensed addition — the entrant's election of 2026-09-22, from the session record: "License both legs".** Two fixed-geometry `scf` runs at the cycle-5 coordinates, starting from the relaxation's retained density, Hubbard occupations and PAW terms (`startingpot = 'file'`; the four files of the retained save directory are pinned by SHA-256 in the specification and copied by the runner into a fresh scratch, the source never written) with fresh wavefunctions, at the production threshold 8.08e-8 Ry and mixing_beta 0.3: one leg with the production `local-TF` mixing, one with `mixing_mode = 'plain'`. Counts, exact: 2 legs, 1 site, 1 geometry, 0 relaxations. Cost (HEA-7): planning 260 core-hours on the observed 128 core-hour fresh-start SCF; ceiling 576 core-hours (288 per job, the 7,200 s SCF cap plus the 900 s projection cap at 128 ranks); scheduler cap 640 core-hours (two tasks at 150 minutes, concurrency two). Supervision (HEA-4): stop when iteration 127 begins or at 7,200 s, KILLED recorded, scratch preserved, no restart. Reading pre-stated: a COMPLETE receipt near −7551.866 Ry means the stagnating density lay in the basin of the fresh-start solution; near −7551.859 Ry, a second self-consistent solution exists; KILLED repeats the production condition from its own density. Scope: numerical diagnostic outcomes only; no relaxed slab, adsorption reference, census result or claim; arrays 20813525 and 20840139 untouched. Decks `runs/hea/lowtail_slab_scf_restart_2026-09-22/`, manifest `runs/m_lowtail_slab_scf_restart_2026-09-22.txt`, specification `results/lowtail_slab_scf_restart_2026-09-22/launch_spec.json`, wrappers `anvil/78_slab_scf_restart.slurm` and `anvil/79_submit_slab_scf_restart.sh`, launched through the guarded phases of `src/dft/lowtail_slab_scf_diag_launch.py` with its batch options.

**2. First generalization row — prepared, NOT LICENSED.** The entrant, asked when the row of four p10-support sites (Cu8Cr23Mn35Co34 seed16 site2 and seed26 site1; Cu26Ni9Cr31Co33 seed1 site0 and seed17 site1; three legs each, twelve legs; expected about 11,400 core-hours on the observed Cu8 basis, 23,300 at ceiling, on top of the 12,100 committed to array tasks 4 to 9) is licensed, answered, from the session record: "Choose whichever is most rigorous and STS pretigious and compelling". Disposition taken under that delegation: the decks are built and preflighted now and carry the builder's NOT LICENSED line; the row is licensed by its own dated line only after the two SCFs above read out, because two of the three Cu8 legs already run have stopped at the ceiling for a cause not yet identified, and a row of relaxations that could repeat that failure at 500 to 1,200 core-hours a leg is neither rigorous nor compelling until the cause is known. Fe25Co25Ni25Cr25 seed2 site0 has no converged fixed-geometry SCF at any mixing tried and enters no new leg. The melt-set disposition of S8 is unchanged by either item.

**Correction of record, 2026-09-22 05:45 UTC, to item 1 above.** The first staging of the licensed batch (stage `slab_scf_restart`, held array 20845265, never released) executed `src/dft/research_batch.py` with the scratch-seed extension added in place, and its stage phase uploaded that modified runner to Anvil. The running relaxation array 20813525 pins the original bytes of that file through its own runner, so its tasks 4 to 9 would have refused to start; none had started. The modified runner was withdrawn from the shared path by the second staging, which restored the original bytes (SHA-256 4195ef7b…) from the pre-stage backup and deployed the extension as its own pinned file `src/dft/research_batch_seeded.py` (SHA-256 ef4116f6…). Attempt 1 is recorded as WITHDRAWN_BEFORE_RELEASE under `results/lowtail_slab_scf_restart_2026-09-22/` and its held job is cancelled. The licensed addition is otherwise unchanged in counts, cost, caps, reading and scope; its paths of record are stage `slab_scf_seeded`, manifest `runs/m_lowtail_slab_scf_seeded_2026-09-22.txt`, specification `results/lowtail_slab_scf_seeded_2026-09-22/launch_spec.json`, wrappers `anvil/78_slab_scf_seeded.slurm` and `anvil/79_submit_slab_scf_seeded.sh`, decks as named above. The local follow-through watcher of array 20813525 stopped on the same pin change at 05:29 UTC and was restarted at 05:38 UTC once the original bytes were back in the worktree.

## 2026-09-22 (second) — Licensed compute addition (A11.R3): one relaxation restart from the low state and two seeded SCFs

**Evidence this line rests on** (docs/research/lowtail-clean-slab-scf-stall-2026-09-19.md, "Discriminating SCFs from the retained density — readout, 2026-09-22 11:33 UTC"). Array 20845364 ran the two licensed SCFs from the retained cycle-5 density of the Cu8Cr23Mn35Co34 seed20 site2 clean-slab relaxation; both were KILLED at their caps, the pre-stated reading being that the production condition repeats from its own density. Read after the fact: the residual sat at 3.4 to 4.5e-7 Ry from the first iteration under both mixings and the energy stayed at the relaxation's stagnating value, stationary to 1e-6 Ry under plain mixing, never approaching the fresh-start state 94.7 meV below. The three retained occupation files differ on one atom of 72: a surface-layer Co (index 20) whose spin-down occupation matrix changes by up to 0.78 with eigenvalues moved by at most 0.026, an orbital reorientation of two minority-spin d electrons that the total moment and the occupation traces do not see.

**Licensed — the entrant's election of 2026-09-22, from the session record: "Restart from low state + seed two more".** Three legs in two batches, both executed by `src/dft/research_batch_seeded.py` with densities copied from retained save directories pinned by SHA-256 (sources never written), HEA-4 supervision (stop when SCF iteration 127 begins, KILLED recorded, scratch preserved, no restart), atomic projector, production mixing 0.3 local-TF, fresh wavefunctions. Counts, exact: 3 legs, 2 sites, 1 relaxation, 2 fixed-geometry SCFs.

1. Stage `slab_relax_seeded`, one leg: the Cu8 clean slab, `calculation = 'relax'`, at the cycle-5 coordinates, density, Hubbard occupations and PAW terms from the low state (array 20840139 task 2, `tmp_slab_c5__b030`), every other line of the production deck unchanged. Question: is the low orbital configuration stable along the remaining relaxation, and does the leg then converge. Cost (HEA-7): planning 590 core-hours (the array's own per-leg planning basis); ceiling 1,667.6 core-hours (46,000 s pw.x cap plus 900 s projection at 128 ranks); scheduler cap 1,664 core-hours (one task, 780 minutes). Reading pre-stated: a COMPLETE receipt reports the first-cycle energy against −7551.86633525 Ry (low state) and −7551.8594 Ry (stagnating state) and whether atom 20's spin-down occupation diagonal is retained; a KILLED receipt records at which cycle and residual the leg stopped. A converged relaxation here is a numerical outcome for the stall question and enters no census, ranking or claim.
2. Stage `slab_scf_seeded2`, two legs: (a) the Cu8 unreconstructed-O adsorbate slab at the killed relaxation's last printed geometry (its nineteenth cycle; array 20813525 task 3), seeded from that leg's retained density, at that leg's own last announced threshold 5.53e-8 Ry; (b) the Fe25Co25Ni25Cr25 seed2 site0 clean slab at its start geometry, seeded from the retained density of the killed 0.10-mixing SCF (array 20840139 task 3), at 8.08e-8 Ry. Question: is the same single-site mechanism at work in those two stalls. Cost: planning 260 core-hours; ceiling 576 (288 per job); scheduler cap 640 (two tasks at 150 minutes, concurrency two). Reading pre-stated as in the first licensed line, plus the atom-resolved occupation comparison against each retained `occup.txt`.

Scope: numerical diagnostic outcomes only; arrays 20813525, 20840139 and 20845364 untouched; the generalization row remains NOT LICENSED. Decks `runs/hea/lowtail_low_state_restart_2026-09-22/`, manifests `runs/m_lowtail_slab_relax_seeded_2026-09-22.txt` and `runs/m_lowtail_slab_scf_seeded2_2026-09-22.txt`, specifications under `results/lowtail_low_state_restart_2026-09-22/{relax,scf}/launch_spec.json`, wrappers `anvil/80_slab_relax_seeded.slurm`, `anvil/81_submit_slab_relax_seeded.sh`, `anvil/82_slab_scf_seeded2.slurm`, `anvil/83_submit_slab_scf_seeded2.sh`, launched through the guarded phases of `src/dft/lowtail_slab_scf_diag_launch.py` with its batch options.

## 2026-09-22 (third) — Program direction after the low-state test: protocol research first, freeze later

**Evidence** (docs/research/lowtail-clean-slab-scf-stall-2026-09-19.md, readouts of 2026-09-22 at 11:33, 15:09 and 18:45 UTC). The relaxation restarted from the low state reproduced that state in three SCF iterations, converged six ionic steps at thresholds down to 4.3e-8 Ry with the Co-20 configuration retained, descended 94 meV, and stalled at the seventh step with a milder residual floor; the two other retained densities are stationary states as well; three of five terminal legs of array 20813525 stalled. The production relaxation protocol carries the electronic state between ionic steps and checks it nowhere.

**The entrant's election of 2026-09-22, from the session record, on the question of how the DFT validation program proceeds: "Protocol research first, freeze later".** Disposition under that election: the S8 melt-set disposition of 2026-09-16 (hold selection; freeze before the first ingot) is unchanged and the freeze moves behind the protocol work; array 20813525 runs out as launched (no leg cancelled or restarted); the generalization row stays NOT LICENSED; a stall-robust relaxation protocol is designed in docs/research/lowtail-stall-robust-protocol-plan-2026-09-22.md, with a two-arm experiment on the Cu8Cr23Mn35Co34 seed20 site2 clean slab (a checked relaxation with a fresh-start comparison at every accepted step and re-seeding at δ = 10 meV; and the seeded relaxation with `mixing_fixed_ns = 15`) that is licensed, if at all, by its own dated line with the counts and costs of that plan's §4. No threshold, U value, projector, magnetic start, cutoff, smearing or kill rule of record changes under this line. The earliest melt-set freeze is thereby the week of Sep 28.

## 2026-09-22 (fourth) — Licensed compute addition (A11.R3): the two-arm protocol experiment on the Cu8 clean slab

**The entrant's election of 2026-09-22, from the session record, on the experiment of docs/research/lowtail-stall-robust-protocol-plan-2026-09-22.md: "Both arms".** Counts, exact: 2 legs, 1 site (Cu8Cr23Mn35Co34 seed20 site2 clean slab), both starting at the cycle-5 coordinates from the low-state density, occupations and PAW terms (array 20840139 task 2), production settings otherwise, HEA-4 supervision on every pw.x process, atomic projector, no threshold, U, projector, magnetic start, cutoff or smearing change.

1. Arm B, stage `slab_relax_fixedns`, one leg `slab_c5low__fixedns15`: the seeded relaxation of array 20851756 repeated with `mixing_fixed_ns = 15` in `&ELECTRONS`, one pw.x process. Cost (HEA-7): planning 590 core-hours; ceiling 1,667.6 (46,000 s pw.x cap plus 900 s projection at 128 ranks); scheduler cap 1,664 (one task, 780 minutes). Reading pre-stated: BFGS converged within the caps, or a stall at a later step than the seventh, each reported with the step, residual and atom 20's spin-down occupation diagonal.
2. Arm A, stage `slab_relax_checked`, one leg `slab_c5low__checked`: a checked relaxation, one BFGS step per segment with the BFGS history continued, a fresh-start SCF at every accepted geometry, re-seeding from the fresh density on a drop of more than δ = 10 meV or on a stall, at most 40 segments and 10 re-seeds. Cost: planning ≈3,600 core-hours; ceiling ≈8,500; scheduler cap 9,216 (one task, 4,320 minutes) or two chained tasks of 2,160 minutes. Reading pre-stated: success is BFGS convergence (total force below the deck's 0.002 Ry/bohr) with every accepted step within δ of its fresh-start check; the readout reports steps, stalls, re-seeds, extra SCFs, final energy and geometry, and atom 20's diagonal per step. Arm A launches only after its driver exists with offline tests, under this same line.

Scope: numerical protocol outcomes only; no census result, adsorption reference or claim; a converged Arm A geometry is a checked trajectory, not a ground-state assertion; arrays 20813525, 20840139, 20845364, 20851753 and 20851756 untouched; the generalization row stays NOT LICENSED.
