...
**Status 2026-09-21.** All seventeen slot positions below are filled by the entrant's adoption directive of 2026-09-21 ("Adopt as proposed", no exception named), the A8 instrument; the authorship event, the two places where the operating decision differs from docs/74's proposal (A10-X, A10-K) and the correction of record that the S5 decks ran on 2026-09-17 before adoption and deposit are in docs/43's AMENDMENT 10 header and its dated addendum of 2026-09-21.
...
|---|---|---|---|---|
| A10-E | `:72` | which written source the P-BEEF criterion is (round-1 vs round-2) | *"the operative P-BEEF criterion is round-2's — its Amendment 10 block **and** its S5 stage spec at `:286-291` together — and round-1's is struck as superseded, with docs/43 `:1930`'s pointer as the reason of record"* (`:72-74`) | `[A10-E 2026-09-21: adopted as proposed — round-2 A10 with its S5 stage spec governs; the round-1 seven-metal rule is superseded (docs/research/research-decisions-2026-09-16.md, row A10-E)]` |
| A10-M | `:84` | the disposition of an outcome in neither band at n = 3 | *"any outcome in neither band maps to **SCORED — MIDDLE BAND / NOT MET** in the A11.R2 vocabulary (docs/43 :2088-2095): reported with its count and its per-metal σ attached, never quoted bare, licensing no registered consequence, and neither HELD, nor TRIGGERED, nor WITHDRAWN-UNSCORED"* (`:84-87`) | `[A10-M 2026-09-21: adopted as proposed — neither band = SCORED — MIDDLE BAND / NOT MET, counts and per-metal σ shown, no binary claim (docs/research/research-decisions-2026-09-16.md, row A10-M)]` |
| A10-D | `:98` | the denominator ladder, fixed before any job, and the non-scoreable rule | *"which ladder applies is fixed by the A10-X election below, before any BEEF job runs, and never afterwards"* (`:98-99`); Ladder B (`:101-108`) and Ladder X (`:110-117`) are reproduced in §2; non-scoreable = *"prints 'convergence NOT achieved', carries an `Error in routine` block, or emits no `BEEFens` line"* (`:125-126`) | `[A10-D 2026-09-21: adopted as proposed — Ladder B, fixed before the S5 executions: n = 3, at least two below 0.25 V confirms, at least two at or above 0.30 V falsifies; one metal reported without verdict, zero unscored (docs/research/research-decisions-2026-09-16.md, row A10-D)]` |
| A10-G | `:150` | closing the S0(a) gate on the record and striking "gated on S0(a)" | *"record gate (a) as **PASSED — SELECT-WINNER = deck (ii) `calculation='ensemble'`** ... and strike 'gated on S0(a)' from docs/43 `:2022` and docs/45 `:33`/`:55`/`:81` by dated erratum. No compute is owed."* (`:150-152`) — see §6 U1 on "no compute is owed" | `[A10-G 2026-09-21: adopted as proposed — S0(a) PASSED for the tested binary (calculation = 'ensemble'); production emission is an execution check, met on Anvil 2026-09-17 (14 decks VALID_BEEF, 2000 members) (docs/research/research-decisions-2026-09-16.md, row A10-G)]` |
| A10-σ | `:162` | the estimator: what σ_BEEF(η) is, longhand | per member *i* from all four states and both gas references at the same index (`:164-166`); ΔG per member by the production CHE ladder with member-independent ZPE/TS (`:167-170`); η_i = max(ΔG)/e − 1.23 V (`:171-174`); *"the **sample standard deviation** of {η_i} over the N members, in volts"* (`:175`); N mismatch across states = non-scoreable (`:178-179`); one script `src/dft/p_beef_readout.py` parsing thresholds out of docs/43 (`:180-183`); stated limit: fixed geometry (`:185-187`) | `[A10-σ 2026-09-21: adopted as proposed — member-wise CHE from four slab states and both gas references at one coefficient row, fixed ZPE/TS, η_i = max ΔG/e − 1.23 V, sample SD (ddof = 1) over 2000 members, limiting-step histogram reported (docs/research/research-decisions-2026-09-16.md, row A10-σ)]` |
| A10-C | `:226` | what σ may be compared against, and both directions pre-stated | bands untouched, < 0.25 / ≥ 0.30 V absolute (`:228`); primary comparison same-metal on η at fixed endpoints U = 0 → 9, *"Ru 0.497, Ti 0.246, Ir 0.027 V"*, max−min beside it, Ir's 7.4× gap stated (`:229-232`); 1.122 V never bare and named as Cr's (`:233-235`); symmetry pair never one end alone (`:236-237`); both directions (`:238-242`) | `[A10-C 2026-09-21: adopted as proposed — absolute bands < 0.25 / ≥ 0.30 V retained; descriptive comparison to same-metal U = 0 → 9 η change (Ru 0.4968, Ti 0.2459, Ir 0.0266 V; Ir grid span 0.1979 V); Cr's 1.122 V always named as Cr's; symmetry pairs keep both coverages (docs/research/research-decisions-2026-09-16.md, row A10-C)]` |
| A10-S | `:253` | scope label of the S5 row | *"the S5 row is labelled **'XC only, non-magnetic three'** ... and σ_BEEF may not be generalised beyond the scoreable set without the extension arm having run"* (`:253-255`) | `[A10-S 2026-09-21: adopted as proposed — primary S5 row labelled XC ensemble at fixed geometry, non-magnetic Ru/Ir/Ti; not a calibrated predictive error or an HEA uncertainty bound (docs/research/research-decisions-2026-09-16.md, row A10-S)]` |
| A10-X | `:257` | **the election:** is Cr added, selecting Ladder X over Ladder B | *"Cr is added, giving {Ru, Ir, Ti, Cr} and Ladder X"* (`:258`), with the against-case at `:262-273` | `[A10-X 2026-09-21: adopted as proposed — no Cr in the primary S5 population; base {Ru, Ir, Ti}, fourteen decks; Cr BEEF+U stays separate research (docs/research/research-decisions-2026-09-16.md, row A10-X)]` — see §2 |
| A10-K | `:388` | P-DISPOSITION by name, the submit-by date, the kill | *"P-BEEF is subject to P-DISPOSITION by name"* (`:389-390`); *"the decks are built and submitted within 7 days of the A10 deposit, and in no case later than Oct 10 2026. Past that, S5 is **CUT**"* (`:391-394`); deliberate withdrawal considered and not taken (`:395-399`) | `[A10-K 2026-09-21: adopted as proposed — bounded capability/gas checks first, then the base program; the Oct 10 date is planning history, not an automatic cut; unfinished at a snapshot = unscored (docs/research/research-decisions-2026-09-16.md, row A10-K)]` |
...
|---|---|
| `[A10-X 2026-09-21: adopted as proposed — no Cr in the primary S5 population; base {Ru, Ir, Ti}, fourteen decks; Cr BEEF+U stays separate research (docs/research/research-decisions-2026-09-16.md, row A10-X)]` — Cr added, or not | if Cr: **four more decks** (Cr is nspin = 2 with `U Cr-3d 3.7`, docs/54 `:81`); the HUBBARD-card evidence is `slab__beefhub.out:1403` (docs/74 `:263-264`); sources would be `runs/probe/Cr_cellsym/{ref__2x1v, s0_O__2x1v_mir, s0_OH__2x1v_mir}` and `runs/s3/Cr/s0_OOH__2x1v_escape` on the `runs/a0/cell/manifest.json:64-68` precedent (the mir relax sits on a saddle 150.8 meV above the escape). Not built (§6 U6). |
| `[A10-D 2026-09-21: adopted as proposed — Ladder B, fixed before the S5 executions: n = 3, at least two below 0.25 V confirms, at least two at or above 0.30 V falsifies; one metal reported without verdict, zero unscored (docs/research/research-decisions-2026-09-16.md, row A10-D)]` — the ladder, in the entrant's words | fixes the denominator for scoring; must precede the first job (`:98-99`) |
...
The remaining entrant items of docs/74 `:438-445` that are not thresholds: `[A10-DISP 2026-09-21: adopted as proposed — P-XU into the six-row body, historical P7 to the appendix with its result unchanged; P-PROJ, P-PLS, P-FLOOR-U, P-SYMCOV, P-BEEF stay in the body (the 2026-09-18 allocation later moved P-SYMCOV to the appendix; that later line governs) (docs/research/research-decisions-2026-09-16.md, row A10-DISP)]`
the body-ledger displacement before Sep 20 (docs/43 `:1930`, docs/74 `:442`); `[A10-A74 2026-09-21: adopted as proposed — an explicit result/status column on the supporting gate table; tested-binary result and production-binary check are separate entries; deposited history not rewritten (docs/research/research-decisions-2026-09-16.md, row A10-A74)]`
whether the A7.4 gate table gains a result column (docs/74 `:443`); `[A10-CHR 2026-09-21: adopted as proposed — no Christensen-style functional-independence or universality claim from three endmembers (docs/research/research-decisions-2026-09-16.md, row A10-CHR)]`
Christensen's functional-independence claim, proposed *not claimed* (docs/74 `:444`);
`[A10-DEP 2026-09-21: adopted as proposed — no additional disclosure or deposit task is an execution gate; historical deposit obligations remain identifiable as history; the A10 deposit of 2026-09-21 is made as such a record (docs/research/research-decisions-2026-09-16.md, row A10-DEP)]` whether the deposit also carries the terminology change (docs/74 `:445`).
