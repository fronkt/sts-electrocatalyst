"""IEEE diagnostic lifecycle: real frozen guards/QC, fake MPI and tiny sources."""
import json
import os
...
from dft import hea_ieee_diagnostic as diag
from test_hea_followup_shell import _pin_script
from test_hea_numerical_shell import batch as numerical_batch, original_sources, run as numerical_run
from test_hea_pilot_runner import ROOT, _shell_path, _write


@pytest.fixture
def diagnostic(numerical_batch):
    case = numerical_batch
    staged = case["runs"].parent
    directory = case["runs"] / diag.DIRECTORY
    directory.mkdir()
    original = case["directory"] / (diag.SOURCE_JOB + ".in")
    ...
    helper = staged / "src/dft/hea_ieee_diagnostic.py"
    helper.write_bytes((ROOT / "src/dft/hea_ieee_diagnostic.py").read_bytes())
    wrapper = case["scripts"] / "59_hea_ieee_rank.sh"
    wrapper.write_bytes((ROOT / "anvil/59_hea_ieee_rank.sh").read_bytes())
    qe = case["root"] / "qe"
    ...
    # realistic finite SCF/Lowdin fixture inherited from numerical lifecycle tests.
    ...
    for name in ("60_hea_ieee_diagnostic.slurm", "61_submit_hea_ieee_diagnostic.sh"):
...
def run(case, submit=False, concurrency="1", **env):
    ...
    # changing it and invoke the same safe subprocess mechanics directly.
    import subprocess
    args = [_shell_path(case["scripts"] / ("61_submit_hea_ieee_diagnostic.sh" if submit
                                           else "60_hea_ieee_diagnostic.slurm"))]
    if submit:
...
def retained(case):
    return case["directory"] / ("tmp_" + diag.SOURCE_JOB) / (diag.PREFIX + ".save")
...
@pytest.mark.parametrize("variable", ["PW", "MPI", "WRAPPER", "FROZEN"])
...
def test_final_acceptance_recheck_controls_runner_success(diagnostic):
    ...
    _pin_script(ROOT / "anvil/60_hea_ieee_diagnostic.slurm",
                diagnostic["scripts"] / "60_hea_ieee_diagnostic.slurm", diagnostic["paths"])
