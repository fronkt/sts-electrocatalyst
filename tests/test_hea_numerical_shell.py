...
SPEC_REL = Path("results/hea_numerical_2026-09-08/launch_spec.json")
RUNNER = "57_hea_numerical.slurm"
SUBMITTER = "58_submit_hea_numerical.sh"
...
def batch(tmp_path):
    ...
    for checkpoint in inventory["checkpoints"]:
        for entry in checkpoint["files"]:
            raw = (checkpoint["prefix"] + ":" + entry["path"] + "\n").encode()
    ...
    for variable, name in (("GUARD", "hea_numerical_guard.py"), ("QC", "hea_followup_qc.py"),
    ...
    env.update(PROJECT=_shell_path(project), MANIFEST=_shell_path(manifest), RUNS=_shell_path(runs),
               QE_PREFIX=_shell_path(qe), PSEUDO_DIR=guard.PSEUDO_DIRECTORY, ACCT="mock", EXCLUDE=exclude,
...
def test_success_retains_independent_full_checkpoint_for_tight_and_recovery(batch, row):
    originals = original_sources(batch)
    result = run(batch, SLURM_ARRAY_TASK_ID=str(row))
    assert result.returncode == 0, result.stdout + result.stderr
    assert "NUMERICAL COMPLETE" in result.stdout
...
def test_submitter_holds_exact_six_job_four_hour_sequential_batch(batch):
    ...
    for flag in ("--hold", "--time=04:00:00", "--no-requeue", "--array=1-6%1"):
...
def test_guard_byte_drift_refused_before_mpi(batch):
    helper = batch["runs"].parent / "src/dft/hea_numerical_guard.py"
