...
requests = json.loads((BASE / 'metadata_requests.json').read_text(encoding='utf-8-sig'))
...
raw_dir = BASE / 'crossref'
raw_dir.mkdir(exist_ok=True)
prior_run = json.loads((BASE / 'crossref_receipts.json').read_text(encoding='utf-8-sig')) if (BASE / 'crossref_receipts.json').exists() else []
...
for item in requests:
    ...
    dump(BASE / 'crossref_receipts.json',receipts)
    print(doi, receipt['status'], flush=True)
    if deferred: break
dump(BASE / 'fetch_summary.json',{'finished_utc':now(),'request_count':len(requests),'receipt_count':len(receipts),'resolved_or_reused':sum(r['status'] in ('DOI_RESOLVED','CACHE_REUSED') for r in receipts),'deferred':deferred,'unrequested':[r['doi'] for r in requests if r['doi'] not in {x['doi'] for x in receipts}],'unresolved':[r['doi'] for r in receipts if r['status'] not in ('DOI_RESOLVED','CACHE_REUSED')],'script_sha256':sha(Path(__file__).read_bytes())})
