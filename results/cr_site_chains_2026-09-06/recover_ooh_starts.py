...
from scripts.screen_diagnostic import identity, sha256_file, validate_manifest, checkpoint, environment
...
def main():
    ...
    with target.open("x", encoding="utf-8") as handle:
        json.dump({"status": "initializing", "arm": args.arm}, handle)
    import torch
    torch.set_num_threads(2)
    runtime = dict(environment("cpu"), threads=2)
    if runtime != data["environment"]:
        raise ValueError("replay runtime differs from initial chain")
    ...
    output = {"schema": "ooh-start-replay-v1", "arm": args.arm, "status": "running",
              ...
              "environment": runtime, "environment_scope": "measured_by_replay_process", "fmax_eV_A": .05, "steps": 300,
