"""One bounded IEEE reproduction, isolated from every original checkpoint.

The shell pins this module and the immutable numerical spec/helpers. Rank capture
...
SPEC = "results/hea_numerical_2026-09-08/launch_spec.json"
DIRECTORY = "hea/ieee_2026-09-09"
MANIFEST = "runs/hea/m_ieee_2026-09-09.txt"
JOB = "hd__leader_builder__atomic__ieee_repro"
SOURCE_JOB = "hn__leader_builder__atomic__tight"
...
def context(runs):
    ...
    spec_raw = frozen._read(frozen._path(root.parent, SPEC))
    spec = frozen.validate_spec(frozen._json(spec_raw))
    candidates = [job for job in spec["jobs"] if job["job"] == SOURCE_JOB]
    if len(candidates) != 1 or candidates[0]["prefix"] != PREFIX or candidates[0]["mode"] != "tight":
        raise ValueError("exact atomic tight source job required")
    original_job = candidates[0]
    inventory_raw = frozen._read(frozen._path(root.parent, spec["source_checkpoints"]))
    if frozen._digest(inventory_raw) != spec["source_checkpoints_sha256"]:
        raise ValueError("source inventory SHA256 mismatch")
    inventory = frozen.validate_inventory(frozen._json(inventory_raw), spec)
    ...
    return dict(root=root, directory=directory, spec=spec, original_job=original_job,
                checkpoint=inventory[PREFIX], source_spec_sha256=frozen._digest(spec_raw))


def _deck(ctx):
    raw = frozen._read(frozen._path(ctx["root"], DIRECTORY + "/" + JOB + ".in"))
    if frozen._digest(raw) != ctx["original_job"]["sha256"]:
        raise ValueError("diagnostic input must be byte-identical to original tight input")
    frozen.validate_deck(raw, ctx["original_job"])
    return raw
...
def prepare(runs):
    ...
    scratch = _member(ctx, "tmp_" + SOURCE_JOB)
    scratch.mkdir()
    ...
    receipt = dict(schema="hea_ieee_clone_v1", job=JOB, source_job=SOURCE_JOB, prefix=PREFIX,
...
def verify_clone_receipt(ctx):
    ...
    expected = dict(schema="hea_ieee_clone_v1", job=JOB, source_job=SOURCE_JOB, prefix=PREFIX,
                    source=ctx["checkpoint"]["dir"],
                    destination=DIRECTORY + "/tmp_" + SOURCE_JOB + "/" + PREFIX + ".save",
                    input_sha256=frozen._digest(runtime),
                    source_spec_sha256=ctx["source_spec_sha256"],
                    source_checkpoints_sha256=ctx["spec"]["source_checkpoints_sha256"],
                    files=ctx["checkpoint"]["files"])
...
def summarize(runs, scf_exit, projection_exit=None):
    ...
    for name in sorted(frozen.PORTABLE_FILES | frozen.WFC_FILES):
        try:
            path = _member(ctx, "tmp_" + SOURCE_JOB + "/" + PREFIX + ".save/" + name)
    ...
    record = dict(schema="hea_ieee_diagnostic_v1", job=JOB, diagnostic_status=diagnosis,
