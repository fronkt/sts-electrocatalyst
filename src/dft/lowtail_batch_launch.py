"""Guarded launch and collection of research batches on Anvil (sibling of the 2026-09-19 diagnostic launcher).

Identical to lowtail_slab_scf_diag_launch.py except that the held-job inspector expects the scheduler's
day form of a time limit at or beyond 24 hours (D-HH:MM:SS).  It pins itself under its own path, so the
original file stays byte-identical for the watches that pin it.
...
LOCAL_SOURCES = (
    "src/dft/lowtail_batch_launch.py", "src/dft/qe_relax_trace.py",
...
def time_limit_field(minutes: int) -> str:
    """scontrol prints HH:MM:SS below one day and D-HH:MM:SS from one day on."""
...
def inspect_held(client, job_id: str) -> dict:
    ...
    needed = dict(JobId=str(job_id), ArrayJobId=str(job_id), WorkDir=PROJECT,
                  **{"CPUs/Task": "1"}, JobState="PENDING", Reason="JobHeldUser", NumCPUs="128", NumTasks="128", Requeue="0",
                  ...
                  TimeLimit=time_limit_field(minutes), Partition="shared", JobName=JOB_NAME)
