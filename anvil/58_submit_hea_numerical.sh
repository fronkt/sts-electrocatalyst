#!/usr/bin/env bash
# Guarded six-job HEA numerical submission; original 47 checks retained.
...
MANIFEST=${1:-$PROJECT/sts/runs/hea/m_numerical_2026-09-08.txt}
CONC=${2:-1}
NP=${NP:-128}
export NP
...
[ "$(basename "$MANIFEST")" = m_numerical_2026-09-08.txt ] || {
...
# used here for its CHECKS only; the launch path is 57_hea_numerical.slurm.
...
sbatch --hold --time=04:00:00 --no-requeue --job-name=hea-numerical -A "$ACCT" -N 1 -n "$NP" ${EXCLUDE:+--exclude="$EXCLUDE"} --array=1-"$N"%"$CONC" \
       --export=ALL,PROJECT,MANIFEST,RUNS,QE_PREFIX,PSEUDO_DIR,NP \
       "$(dirname "$0")/57_hea_numerical.slurm"
