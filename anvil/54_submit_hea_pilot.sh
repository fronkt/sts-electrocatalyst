#!/usr/bin/env bash
# Guarded two-job HEA pilot submission; original 47 checks retained.
...
MANIFEST=${1:-$PROJECT/sts/runs/hea/m_controls_2026-09-07_pilot_approved.txt}
CONC=${2:-1}
NP=${NP:-128}
export NP
...
[ "$NP" = 128 ] && [ "$CONC" = 1 ] || { echo "REFUSE: pilot requires NP=128 CONC=1" >&2; exit 2; }
[ "$(basename "$MANIFEST")" = m_controls_2026-09-07_pilot_approved.txt ] || {
  echo "REFUSE: pilot requires the exact approved manifest filename" >&2; exit 2;
}
...
[ "$N" -eq 2 ] || { echo "REFUSE: pilot requires exactly two runnable rows" >&2; exit 2; }
expected_rows=$'hea/controls_2026-09-07 hc__leader_builder__atomic__baseline .in 8\nhea/controls_2026-09-07 hc__leader_pull2.10__atomic__baseline .in 8'
[ "$(cat "${MANIFEST}.lines")" = "$expected_rows" ] || {
  echo "REFUSE: rows differ from the two exact approved endpoints and nk8" >&2; exit 2;
}
...
while read -r d job suf nk extra; do
  ...
  case "$job" in
    hc__leader_builder__atomic__baseline) expected_sha=d931c2eda18ca90a400e909f965a4190baa2357bfcb55cd0359a140f0a4ac0b6;;
    hc__leader_pull2.10__atomic__baseline) expected_sha=aec1696257df44dada98eb071384c3deb45ceb51b90f4b814a22c1953af2ce37;;
    *) echo "REFUSE: unexpected pilot job" >&2; exit 2;;
  esac
  actual_sha=$(sha256sum "$RUNS/$d/${job}${suf}" | awk '{print $1}')
  [ "$actual_sha" = "$expected_sha" ] || { echo "REFUSE: input SHA256 differs for $job" >&2; exit 2; }
  for prior in "$RUNS/$d/$job.out" "$RUNS/$d/$job.projwfc.out" "$RUNS/$d/$job.run.in" "$RUNS/$d/$job.projwfc.in" "$RUNS/$d/tmp_$job" "$RUNS/$d/dens/$job.save" "$RUNS/$d/dens/$job.save.new"; do
    if [ -e "$prior" ] || [ -L "$prior" ]; then
      echo "REFUSE: preexisting pilot artifact $prior; no automatic retry" >&2; exit 2
    fi
  done
done < "${MANIFEST}.lines"
...
# used here for its CHECKS only; the launch path is 53_hea_pilot.slurm.
...
echo "== submitting HELD pilot array 1-$N%$CONC ($NP cores each); inspect before release"
echo "   requested four-hour cores*time bound: $((N * NP * 4)) core-hours (1024 for both jobs)"
...
sbatch --hold --time=04:00:00 --no-requeue --job-name=hea-pilot -A "$ACCT" -N 1 -n "$NP" ${EXCLUDE:+--exclude="$EXCLUDE"} --array=1-"$N"%"$CONC" \
       --export=ALL,MANIFEST,RUNS,QE_PREFIX,PSEUDO_DIR,NP \
       "$(dirname "$0")/53_hea_pilot.slurm"
