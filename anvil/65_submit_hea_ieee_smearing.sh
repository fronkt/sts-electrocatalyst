#!/usr/bin/env bash
# Guarded one-job HEA smearing diagnostic submission; original 47 checks retained.
...
MANIFEST=${1:-$PROJECT/sts/runs/hea/m_ieee_smearing_2026-09-10.txt}
CONC=${2:-1}
NP=${NP:-128}
export NP
...
[ "$NP" = 128 ] && [ "$CONC" = 1 ] || { echo "REFUSE: smearing diagnostic requires NP=128 CONC=1" >&2; exit 2; }
[ "$(basename "$MANIFEST")" = m_ieee_smearing_2026-09-10.txt ] || {
  echo "REFUSE: smearing diagnostic requires the exact approved manifest filename" >&2; exit 2;
}
...
GUARD="$ROOT/src/dft/hea_ieee_smearing_diagnostic.py"
SENSITIVITY="$ROOT/src/dft/hea_sensitivity_guard.py"
WRAPPER="$ROOT/anvil/hea_ieee_smearing_rank.sh"
...
check_hash "$MANIFEST" 4fb9f7c92844f6ba85501fd64030d1be1abfef73a1443d5d78d2afe8415d5a1b
check_hash "$FROZEN" 98d866553566d9e8ab660535f5bbc0c33228f5160df59eda9f41de9c85ee5f9a
check_hash "$GUARD" 820fa225b5ea72ada09cda80d4929332754b1106f87c52417504f6eab590d813
...
check_hash "$WRAPPER" 94f7290f5b2887b022cfa2363dea3301a0078491a22c03993c683fa1ff7dedfb
...
[ "$MANIFEST" = "$ROOT/runs/hea/m_ieee_smearing_2026-09-10.txt" ] || { echo "REFUSE: unexpected manifest path"; exit 2; }
...
# used here for its CHECKS only; the launch path is 64_hea_ieee_smearing.slurm.
...
echo "== submitting HELD smearing diagnostic array 1-$N%$CONC ($NP cores each); inspect before release"
...
sbatch --hold --mem=237G --time=04:00:00 --no-requeue --job-name=hea-ieee-smearing -A "$ACCT" -N 1 -n "$NP" ${EXCLUDE:+--exclude="$EXCLUDE"} --array=1-"$N"%"$CONC" \
       --export=ALL,PROJECT,MANIFEST,RUNS,QE_PREFIX,PSEUDO_DIR,NP \
       "$(dirname "$0")/64_hea_ieee_smearing.slurm"
