#!/usr/bin/env bash
# Guarded fourteen-job HEA follow-up submission; original 47 checks retained.
...
MANIFEST=${1:-$PROJECT/sts/runs/hea/m_controls_2026-09-07_followup_approved.txt}
CONC=${2:-1}
NP=${NP:-128}
export NP
...
[ "$NP" = 128 ] && [ "$CONC" = 1 ] || { echo "REFUSE: followup requires NP=128 CONC=1" >&2; exit 2; }
[ "$(basename "$MANIFEST")" = m_controls_2026-09-07_followup_approved.txt ] || {
  echo "REFUSE: followup requires the exact approved manifest filename" >&2; exit 2;
}
...
SPEC="$ROOT/results/hea_followup_2026-09-07/launch_spec.json"
GUARD="$ROOT/src/dft/hea_followup_guard.py"
...
check_hash "$SPEC" e4c3af91cfce429bfc7b4e79e62059a3343630cbf85d5bc60d20e7353804eae5
check_hash "$GUARD" b9a7ce86c30a3f3b66619c22e5d86b086dba89aabe736d59d30c6967748e33d3
...
[ "$MANIFEST" = "$ROOT/runs/hea/m_controls_2026-09-07_followup_approved.txt" ] || { echo "REFUSE: unexpected manifest path"; exit 2; }
...
# used here for its CHECKS only; the launch path is 55_hea_followup.slurm.
...
echo "== submitting HELD followup array 1-$N%$CONC ($NP cores each); inspect before release"
echo "   requested four-hour cores*time bound: $((N * NP * 4)) core-hours (7168 for fourteen jobs)"
...
sbatch --hold --time=04:00:00 --no-requeue --job-name=hea-followup -A "$ACCT" -N 1 -n "$NP" ${EXCLUDE:+--exclude="$EXCLUDE"} --array=1-"$N"%"$CONC" \
       --export=ALL,PROJECT,MANIFEST,RUNS,QE_PREFIX,PSEUDO_DIR,NP \
       "$(dirname "$0")/55_hea_followup.slurm"
