#!/usr/bin/env bash
# Submit an A0 manifest (block 6A / A6.1) as a Slurm array at the A8.6 launch
# shape: 128 ranks, -nk per row, -N 1, 48 h. Identical to 43_submit_s3_wave1.sh
# except that the runner is 46_a0.slurm, which runs projwfc.x in the same job.
#
#   bash 47_submit_a0.sh [manifest] [max_concurrent]
#
# WHY A SEPARATE SUBMITTER. A6.5(1) requires every A0 point to retain its .save
# or run projwfc.x inline. 42_s3_wave1.slurm retains the save MINUS wfc* -- right
# for S3, which forbids wavefunction restarts, and wrong for A0, because
# projwfc.x needs exactly those wavefunctions. 46_a0.slurm projects before the
# trim. Everything else -- the PARITY_PASS gate, the pseudo md5 preflight, the
# driver's dry preflight, the stale-.out refusal -- is 43's, unchanged.
#
# REFUSES unless $PROJECT/parity/PARITY_PASS exists (docs/43:1613-1621).
# No FORCE override: every deck in an A0 manifest is bank-bound.
set -euo pipefail

MANIFEST=${1:-$PROJECT/sts/runs/a0/m_a0cell.txt}
CONC=${2:-6}
NP=${NP:-128}
export NP
...
# used here for its CHECKS only; the launch path is 46_a0.slurm.
