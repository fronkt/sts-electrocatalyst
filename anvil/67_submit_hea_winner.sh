#!/usr/bin/env bash
# Guarded eight-job HEA winner audit submission; original 47 checks retained.
...
MANIFEST=${1:-$PROJECT/sts/runs/hea/m_winner_2026-09-10.txt}
CONC=${2:-1}
NP=${NP:-128}
export NP
...
[ "$NP" = 128 ] && [ "$CONC" = 1 ] || { echo "REFUSE: winner requires NP=128 CONC=1" >&2; exit 2; }
[ "$(basename "$MANIFEST")" = m_winner_2026-09-10.txt ] || {
  echo "REFUSE: winner requires the exact approved manifest filename" >&2; exit 2;
}
...
if [ ! -f "$PROJECT/parity/PARITY_PASS" ]; then
  ...
  echo "        The established QE reference build must pass parity." >&2
  exit 2
fi
...
# Exclude every known faulty node in the frozen manifest.
...
"$PYTHON" "$GUARD" --spec "$SPEC" --runs "$RUNS" --pseudo-dir "$PSEUDO_DIR" || exit 2
...
# used here for its CHECKS only; the launch path is 66_hea_winner.slurm.
...
echo "== submitting HELD winner array 1-$N%$CONC ($NP cores each); inspect before release"
echo "   requested four-hour cores*time bound: $((N * NP * 4)) core-hours (4096 for eight jobs)"
...
sbatch --hold --time=04:00:00 --mem=237G --no-requeue --job-name=hea-winner -A "$ACCT" -N 1 -n "$NP" ${EXCLUDE:+--exclude="$EXCLUDE"} --array=1-"$N"%"$CONC" \
       --export=ALL,PROJECT,MANIFEST,RUNS,QE_PREFIX,PSEUDO_DIR,NP \
       "$(dirname "$0")/66_hea_winner.slurm"
